...
rules.add(new Rule("age factor", profile -> true, (profile, premium) -> {
    ...
    BiConsumer<DriverProfile, Premium> ageFactorAction = (p, premiumObj) -> {
        // Get the driver's age
        int age = p.getAge();
        
        // Initialize factor to 1.0 by default
        double factor = 1.0;
        
        // Initialize explanation for the age factor
        String explanation;

        // Apply different factors based on the driver's age
        if (age < 20) {
        ...
        // Calculate the adjustment for the premium based on the factor
        double adjustment = premiumObj.getBaseRate() * (factor - 1.0);
        
        // Add the adjustment to the premium
        premiumObj.addAdjustment("Age factor", adjustment, explanation);
    };

    // Apply the action to the profile and premium
    ageFactorAction.accept(profile, premium);
}));
        ...
        Predicate<DriverProfile> accidentPredicate = p -> p.getAccidentsInLastFiveYears() > 0;
